...
exports.parseRtpPacket = function parseRtpPacket(arrayBuf) {
    ...
    const firstByte = buf.getUint8(0),
        secondByte = buf.getUint8(1);
    ...
    for (let i = 0; i < parsed.csrcCount; i++) {
        parsed.csrc.push(buf.getUint32(9 + 4 * i));
    }

    parsed.payload = arrayBuf.slice(FIXED_HEADER_LENGTH + 4 * parsed.csrcCount);

    return parsed;
};

const payloadTypesHash = {
